<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-full">
                    ...
                    <form action="{{ route('sales.update', $sale->id) }}" method="POST" id="sale-form" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <section>
                            <header>
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    Altere o cliente ou os produtos desta venda.
                                </p>
                            </header>
                                ...
                                <div>
                                    ...
                                    <select name="client_id" id="client_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                        ...
                                        @foreach($clients as $client)
                                            {{-- Pré-seleciona o cliente da venda --}}
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full bg-white dark:bg-gray-800">
                                ...
                                <tbody id="sale-items-table" class="divide-y divide-gray-200 dark:divide-gray-700">
                                    {{-- Pré-popula os itens existentes --}}
                                    @foreach($sale->items as $item)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200" data-product-id="{{ $item->product_id }}">
                                        <td class="py-4 px-6">{{ $item->product->name }}<input type="hidden" name="items[{{ $item->product_id }}][product_id]" value="{{ $item->product_id }}"></td>
                                        <td class="py-4 px-6 text-center"><input type="number" name="items[{{ $item->product_id }}][quantity]" value="{{ $item->quantity }}" min="1" class="w-20 text-center border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm item-quantity" data-price="{{ $item->unit_price }}"></td>
                                        <td class="py-4 px-6 text-right">R$ {{ number_format($item->unit_price, 2, ',', '.') }}</td>
                                        <td class="py-4 px-6 text-right subtotal">R$ {{ number_format($item->unit_price * $item->quantity, 2, ',', '.') }}</td>
                                        <td class="py-4 px-6 text-center"><button type="button" class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 remove-item-btn"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="text-right">
                            <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Total: <span id="total-amount-display">R$ {{ number_format($sale->total_amount, 2, ',', '.') }}</span></h3>
                            <input type="hidden" id="total_amount_hidden" value="{{ $sale->total_amount }}">
                        </div>
                        ...
                        <section>
                            <header>
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    Altere a forma de pagamento e gere um novo plano de parcelas se necessário.
                                </p>
                            </header>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <select name="payment_method_id" id="payment_method_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                        @foreach($paymentMethods as $pm)
                                            <option value="{{ $pm->id }}" @selected(old('payment_method_id', $sale->payment_method_id) == $pm->id)>{{ $pm->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="sm:col-span-1">
                                    <x-input-label for="installments_count_input" value="Nº Parcelas" />
                                    <x-text-input type="number" id="installments_count_input" value="{{ $sale->installments_count }}" min="1" class="mt-1 block w-full"/>
                                </div>
                                <div class="sm:col-span-2">
                                    <x-input-label for="first_due_date" value="1º Vencimento" />
                                    <x-text-input type="date" id="first_due_date" value="{{ \Carbon\Carbon::parse($sale->installments->first()->due_date)->format('Y-m-d') }}" class="mt-1 block w-full"/>
                                </div>
                                <div class="sm:col-span-1">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full bg-white dark:bg-gray-800">
                                ...
                                <tbody id="installments-table" class="divide-y divide-gray-200 dark:divide-gray-700">
                                    {{-- Pré-popula as parcelas existentes --}}
                                    @foreach($sale->installments as $installment)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                        <td class="py-4 px-6 text-center">{{ $installment->installment_number }}<input type="hidden" name="installments[{{ $loop->index }}][installment_number]" value="{{ $installment->installment_number }}"></td>
                                        <td class="py-4 px-6"><input type="date" name="installments[{{ $loop->index }}][due_date]" value="{{ \Carbon\Carbon::parse($installment->due_date)->format('Y-m-d') }}" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"></td>
                                        <td class="py-4 px-6 text-right"><input type="number" name="installments[{{ $loop->index }}][value]" value="{{ number_format($installment->value, 2, '.', '') }}" step="0.01" class="w-32 text-right border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="flex items-center gap-4 mt-6">
                            <x-primary-button>Atualizar Venda</x-primary-button>
    ...
    <script>
        // O SCRIPT JAVASCRIPT É EXATAMENTE O MESMO DA TELA DE CRIAÇÃO
